 ...
 * <p>
 * 8 数字推盘难题. 8 数字推盘难题是一个由 S.Loyd 于 1870s 设计的游戏.他是在一个 3 乘 3 的网格中填入
 * 上面标明数字 1 到 8 的8 个方形板以及一个空白格.你的目标是重新排列板使得他们是有序的.你可以将一个板横向或者纵向
 * 滑动到空白的地方不可以对角滑动.编写一个程序使用 A* 算法解决这个难题.使用总的需要移动多少次来到达这个板的位置加上
 * 剩余错误位置的板的数量作为优先级.(注意,你必须移动的次数至少和错误位置的板的数量是一样的.)调查其他替代错误位置
 * 板数量的方式,比如使用每个板距离正确位置的 Manhattan 距离的和,或者是这些距离的平方和
 *
 * @author LeonChen
 * @since 1/12/20
 */
class E02_05_32 {
    ...
    /**
     * A* 算法是 Dijkstra 算法和最佳优先算法的一种结合. 基本概念就是通过路径长度和一个估算到终点成本的公式
     * 来得出一条最佳路径.
     * 8puzzle 有一个特殊情况,就是当一个可以形成最终结果的原始图中,交换两个非空板的话这个时候就无法达到最终解
     * TODO 后续写出这个算法的详细分析
     *
     * @param args
     */
    public static void main(String[] args) {
        E02_05_32 puzzle = new E02_05_32();
    ...
    private List<Node> getNeighbors(Node node) {
        ...
        switch (emptyCellIndex) {
            ...
            case 8:
                ...
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + emptyCellIndex);
        }
