 ...
 * SeparateChainingST and BinarySearchST as a starting point, develop implementations
 * SeparateChainingMultiST and BinarySearchMultiST for these APIs.
 * <p>
 * 符号表中的相等键.思考 MultiST(无序或有序)的 API 和我们 363 页和 366 页的符号表 API 相同,但是允许使用
 * 相同的键,使得 get() 的作用是返回任意给定键关联的值,并且我们添加一个新方法
 * Iterable<Value> getAll(Key key)
 * 它返回所有给定键关联的值.使用我们 SeparateChainingST 和BinarySearchST的代码作为起始,开发这些 API
 * 的实现 SeparateChainingMultiST 和 BinarySearchMultiST
 *
 * @author LeonChen
 * @since 9/11/21
 */
class E3_5_19 {
    ...
    public static void main(String[] args) {
        testSeparateChainingMultiST();
        testBinarySearchMultiST();
    }

    public static class SeparateChainingMultiST<Key, Value> {
    ...
    private static void testSeparateChainingMultiST() {
    ...
    private static void testBinarySearchMultiST() {
