...
/**
 * 3.5.18 Multisets. After referring to Exercises 3.5.2 and 3.5.3 and the previous
 * exercise, develop APIs MultiHashSET and MultiSET for multisets (sets that can have
 * equal keys) and implementations SeparateChainingMultiSET and BinarySearchMultiSET
 * for multisets and ordered multisets, respectively.
 * <p>
 * 多重集合. 在做了练习 3.5.2 和 3.5.3 以及前面的练习,开发 API MultiHashSET 和 MultiSET 给 multisets
 * (集合可以拥有相同的键) 并且分别实现多重集合和有序集合的 SeparateChainingMultiSET 和
...
class E3_5_18 {
    ...
    /**
     * @formatter:off
     * 添加的时候不判断唯一性,删除的时候遍历所有 key
     * @formatter:on
     */
    public static void main(String[] args) {
        testSeparateChainingMultiSET();
        testBinarySearchMultiSET();
    }
    ...
    public static class SeparateChainingMultiSET<Key> implements MultiHashSET<Key> {
    ...
    public static class BinarySearchMultiSET<Key extends Comparable<Key>> implements MultiSET<Key> {
    ...
    private static void testSeparateChainingMultiSET() {
    ...
    private static void testBinarySearchMultiSET() {
