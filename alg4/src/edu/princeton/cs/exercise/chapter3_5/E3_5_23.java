...
/**
 * 3.5.23 Sparse matrices. Develop an API and an implementation for sparse 2D matrices.
 * Support matrix addition and matrix multiplication. Include constructors for row
 * and column vectors.
 * <p>
 * 稀疏矩阵.为稀疏 20 矩阵开发一个 API 和一个实现.支持矩阵加法和矩阵乘法.包含给行列矢量的构造器
 *
 * @author LeonChen
 * @since 9/12/21
 */
class E3_5_23 {
    ...
    public static void main(String[] args) {
        ...
        SparseMatrix sparseMatrixA = new SparseMatrix(2, 2);
        SparseMatrix sparseMatrixB = new SparseMatrix(2, 2);
        SparseMatrix sparseMatrixC = new SparseMatrix(2, 3);
        SparseMatrix sparseMatrixD = new SparseMatrix(2, 3);
    ...
    public static class SparseMatrix implements SparseMatrixAPI {
        ...
        public SparseMatrix dot(SparseVector sparseVector) {
            if (columnSize != sparseVector.dimension) {
                throw new IllegalArgumentException("Matrix A columns number and Vector " +
                        "B rows number must match");
            }
            SparseMatrix result = new SparseMatrix(rowSize, sparseVector.dimension);
            for (int i = 0; i < rows.length; i++) {
                for (int j = 0; j < sparseVector.dimension; j++) {
