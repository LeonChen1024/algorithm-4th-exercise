 ...
 * <p>
 * LRU 缓存.创建一个数据结构支持以下操作: 访问和移除.访问操作如果不存在的时候就插入这个项.移除操作删除并且
 * 返回最近访问的值.提示:使用双向链表按照访问顺序维护项,并用指针指向第一个和最后一个节点.使用一个符号表其中
 * key= 项,值= 链表中的位置.当你访问一个元素的时候,删除这个节点并重新插入到起始位置.当你移除一个元素,从链表和
 * 符号表中删除它
 *
 * @author LeonChen
 * @since 9/13/21
...
class E3_5_26 {
    ...
    public static class LRUCache<Item> {

        private LinkedList<Item> doublyLinkedList;
        private SeparateChainingHashST<Item, LinkedList.Node> hashTable;
        ...
        public void access(Item item) {
            ...
            if (hashTable.contains(item)) {
                LinkedList.Node itemNodeInList = hashTable.get(item);
                doublyLinkedList.unlink(itemNodeInList);
            }
            doublyLinkedList.addFirst(item);
            LinkedList.Node newListNode = doublyLinkedList.first;
        ...
        //O(1)
        public Item remove() {
            Item leastRecentlyAccessedItem = doublyLinkedList.removeLast();
