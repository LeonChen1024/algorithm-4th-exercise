 ...
 * with a black link when inserting into a 3-node at the bottom. Run experiments to
 * develop a hypothesis estimating the average path length in a tree built from N random
 * insertions.
 * <p>
 * 没有平衡限制的 2-3 树.开发一个基础符号表 API ,使用 2-3 树结构实现并且不需要平衡.允许 3 节点出现在每一边.
 * 将新节点挂到底部的 3-节点时使用黑链接.运行试验来验证 N 次随机插入到一个树构建的平均路径长度
 *
 * @author LeonChen
 * @since 6/26/21
 */
class E3_3_23 {
    ...
    public static void main(String[] args) {
        TwoThreeTreeNonBalanced<Integer, Integer> nonBalanced = new TwoThreeTreeNonBalanced<>();
        nonBalanced.put(5, 1);
        nonBalanced.put(2, 1);
        nonBalanced.put(8, 1);
        nonBalanced.put(9, 1);
        nonBalanced.put(7, 1);
        nonBalanced.put(0, 1);
        StdOut.println(nonBalanced.averagePathLength());

    }

    private static class TwoThreeTreeNonBalanced<Key extends Comparable<Key>, Value> {
         ...
         */
        public TwoThreeTreeNonBalanced() {
        }
        ...
        private Node put(Node h, Key key, Value val, Node parent) {
            if (h == null) {
                ...
                } else if (isRed(parent) || isRed(parent.left) || isRed(parent.right)) {
                    return new Node(key, val, BLACK, 1);
                } else {
