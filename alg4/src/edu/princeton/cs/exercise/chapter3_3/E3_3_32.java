...
/**
 * 3.3.32 AVL trees. An AVL tree is a BST where the height of every node and that of its
 * sibling differ by at most 1. (The oldest balanced tree algorithms are based on using
 ...
 * balanced) 2-3-4 tree, where red links are not necessarily left-leaning. Extra
 * credit:  Develop an implementation of the symbol-table API that uses this as the
 * underlying data structure. One approach is to keep a height field in each node,
 * using rotations after the recursive calls to adjust the height as necessary; another
 * is to use the red-black representation  and use methods like moveRedLeft() and
 * moveRedRight() in Exercise 3.3.39 and Exercise 3.3.40.
 * <p>
 * AVL 树. AVL 树是一个每个节点和他的兄弟高度最多差 1 的节点.(最原始的平衡树算法是基于旋转来维持 AVL 树的
 * 平衡.)证明在一个 AVL 树中偶数高度到奇数高度的节点是一个 2-3-4 树的红色连接(完美平衡),其中红色链接不一定
 * 要在左侧. 额外信息: 开发一个符号表 API 实现使用这个数据结构.一种方式是将高度字段保存在每个节点中,在每次
 * 递归调用后使用旋转来调整高度;另一种是使用红黑树来表示并使用练习 3.3.39 和练习 3.3.40 中的方法
 * moveRedLeft() 和 moveRedRight()
 *
 * @author LeonChen
 * @since 7/4/21
 */
