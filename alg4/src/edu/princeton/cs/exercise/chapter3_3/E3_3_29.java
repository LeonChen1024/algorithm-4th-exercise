 ...
 * for the color bit, based on the following trick: To color a node red, swap its two
 * links. Then, to test whether a node is red, test whether its left child is larger
 * than its right child. You have to modify the compares to accommodate the possible link
 * swap, and this trick replaces bit compares with key compares that are presumably
 * more expensive, but it shows that the bit in the nodes can be eliminated, if necessary.
 * <p>
 * 额外存储.修改 RedBlackBST 使得它不需要任何用于存储颜色的空间.然后,为了测试哪个节点是红色的,测试它的左
 * 子是否比右子大.你需要修改对比来完成链接交换,这个技巧替换了空间为使用对比键可能会消耗更多的资源,但是它证明了
 * 如果需要的话节点的颜色位是可以省略的
 *
 * @author LeonChen
 * @since 7/1/21
...
class E3_3_29 {
    ...
    /**
     * @formatter:off
     * 关键在于红色链接互换兄弟树,搜索时判断红节点来找到对应的向下路径
     * @formatter:on
     */
    public static void main(String[] args) {
        ...
        RedBlackBSTOptimalStorage<Integer, Integer> redBlackBSTOptimalStorage = new RedBlackBSTOptimalStorage<>();
    ...
    private static class RedBlackBSTOptimalStorage<Key extends Comparable<Key>, Value> {
