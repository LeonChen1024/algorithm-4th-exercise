 ...
 * <p>
 * 线性化.你的目标是提供一个扩展了 API 的ThreadedST 在常量时间内支持下列额外操作:
 * Key next(Key key)         key that follows key (null if key is the maximum)
 * Key prev(Key key)         key that precedes key (null if key is the minimum)
 * 为了这么做,添加字段 pred 和 succ 到节点内维护前驱和后继的节点连接,并且修改 put(), deleteMin(),
 * deleteMax(), 和 delete() 来维护这些字段
 *
 * @author LeonChen
 * @since 4/26/21
 */
class E3_2_34 {
    ...
    private static class ThreadedST<Key extends Comparable<Key>, Value> {
        ...
        public void put(Key key, Value val) {
            ...
            root = put(root, key, val, 0, null, null);
            assert check();
        }

        private Node put(Node x, Key key, Value val, int depth, Node pred, Node succ) {
            if (x == null) {
            ...
            int cmp = key.compareTo(x.key);
            if (cmp < 0) x.left = put(x.left, key, val, depth + 1, pred, x);
            else if (cmp > 0) x.right = put(x.right, key, val, depth + 1, x, succ);
            else x.val = val;
        ...
        private Node deleteMin(Node x, boolean updatePredSucc) {
            if (x == null) return null;
            if (x.left == null) {
                if (updatePredSucc && x.succ != null) {
                    x.succ.pred = null;
                }
            ...
            x.left = deleteMin(x.left, updatePredSucc);
        ...
        private Node delete(Node x, Key key, int depth) {
            ...
            if (cmp < 0) x.left = delete(x.left, key, depth + 1);
                ...
                if (x.left == null || x.right == null) {
                    ...
                    Node aux = x;
                    x = min(aux.right);
                    x.right = deleteMin(aux.right, false);
                    x.left = aux.left;
