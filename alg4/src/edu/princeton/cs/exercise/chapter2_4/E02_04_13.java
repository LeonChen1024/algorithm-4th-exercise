...
class E02_04_13 {
  ...
  /**
   * 主要修改两个地方即可,首先将 sink()中的 j<n 判断直接删除,如下<code>
   *         // if (j < n && less(j, j + 1)) j++;
   *         if (less(j, j + 1)) j++;
   * </code> <br>
   * 这个时候因为少了判断 j<n 有可能出现越界的情况,<br>
   * 接下来我们将delMax()中的 pq[n + 1] = null 修改为 pq[n + 1] = pq[1]; 如下<code>
   *       // pq[n + 1] = null;
   *       pq[n + 1] = pq[1]; </code> <br>
   * 这么做的目的是为了直接给n+1 一个哨兵位, 即使对比到了这里,由于哨兵位的存在也不会继续往下对比了
   */
  ...
  public class MaxPQ<Key> implements Iterable<Key> {
    ...
    public Key delMax() {
      ...
      pq[n + 1] = pq[1]; // to avoid loitering and help with garbage collection
