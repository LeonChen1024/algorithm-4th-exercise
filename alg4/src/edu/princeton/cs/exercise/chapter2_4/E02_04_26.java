 ...
 *
 * <p>没有交换的堆.因为 exch() 方法在 sink() 和 swim() 操作中有使用,项的存储和加载消耗了两倍的成本.给出一个<br>
 * 更加有效的实现避免这个低效的操作,一个 la 插入排序(见练习 2.1.25)
 *
 * @author LeonChen
 * @since 10/22/20
 */
class E02_04_26 {
  /**
   * 也就是不要每次都移动当前对比的元素,而是等到找到他最后的位置再进行交换
   *
   * @param <Key>
   */
  public static class MaxPQ<Key extends Comparable> implements Iterable<Key> {
