...
/**
 * 1.3.16 Using readInts() on page 126 as a model, write a static method readDates() for Date that
 * reads dates from standard input in the format specified in the table on page 119 and returns an
 * array containing them.
 *
 * <p>使用126页的 readInts() 方法作为模型,给 Date 编写一个静态方法 readDates() 从标准输入中读取按照 119页的格式<br>
 * 的数据并且返回一个数组包含他们.
 *
 * @author LeonChen
 * @since 1/6/20
...
class e01_03_16 {
  ...
  public static void main(String[] args) {
    String s = "1/1/2020 1/6/2020 1/10/2020";
  ...
  public static class Date implements Comparable<Date> {
    ...
    public static Date[] readDates(String s) {
      ...
      return result;
      // 因为题目让我们使用 queue 来实现,应该是出于熟悉 queue的想法,否则可以直接用数组
      //      String[] dates = s.split(" ");
      //      int n = dates.length;
      //
      //      Date[] result = new Date[n];
      //      for (int i = 0; i < n; i++) {
      //        result[i] = new Date(dates[i]);
      //      }
      //
      //      return result;
    }
